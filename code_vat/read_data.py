...
class Translator:
    ...
    def __call__(self, ori, idx):
        #out1 = self.de[idx]
        #out2 = self.ru[idx]
        #out2 = ru2en.translate(en2ru.translate(train_text[idxs[id]],  sampling = True, temperature = 0.9),  sampling = True, temperature = 0.9)
        out1 = self.de_new[idx]
        out2 = self.ru_new[idx]
...
def get_data(data_path, n_labeled_per_class, unlabeled_per_class=5000, max_seq_len=256, model='bert-base-uncased', train_aug=False):
    ...
    with open(data_path + 'de_1.pkl', 'rb') as f:
            de = pickle.load(f)
    ...
    #--------------- half the data to avoid memory error ---------------------
    train_df = pd.read_csv(data_path+'train.csv', header=None)
    train_len = len(train_df)-1
    #train_df = train_df[:train_len//2]
    train_df = train_df.loc[de_keys]

    test_df = pd.read_csv(data_path+'test.csv', header=None)
    test_len = len(test_df)-1
    test_df = test_df[:test_len//6]
...
def train_val_split(labels, n_labeled_per_class, unlabeled_per_class, n_labels, seed=0):
    ...
    for i in range(n_labels):
        ...
        if n_labels == 2:
            ...
            val_idxs.extend(idxs[-200:])  # CHANGED: 2000-->200
        else:
            # Yahoo/AG News
            train_pool = np.concatenate((idxs[:500], idxs[5500:-2000]))
            train_labeled_idxs.extend(train_pool[:n_labeled_per_class])
