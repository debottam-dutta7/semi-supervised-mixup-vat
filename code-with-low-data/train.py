...
from mixtext_vat import MixText #-------------------------
from vat_utils import vat_loss #---------
from transformers.modeling_bert import BertEmbeddings, BertPooler, BertLayer
from pytorch_transformers import * 

sys.stdout = io.TextIOWrapper(open(sys.stdout.fileno(), 'wb', 0), write_through=True) # for no buffering in printing 
...
parser.add_argument('--gpu', default='0', type=str,         # '0,1,2,3'-->'0'
                    help='id(s) for CUDA_VISIBLE_DEVICES')

parser.add_argument('--n-labeled', type=int, default=20,
                    help='number of labeled data')

parser.add_argument('--un-labeled', default=500, type=int,
...
parser.add_argument('--data-path', type=str, default='/home/debottamd/course_work/DL-NLP/Project/Dataset/yahoo_answers_csv/', #----> changed
                    help='path to data folders')

parser.add_argument('--mix-layers-set', nargs='+',
                    default=[7,9,12], type=int, help='define mix layer set') # [0,1,2,3]-->[7,9,12]
...
parser.add_argument('--use-vat', default=False, type=bool,
                    help='use virtual adversarial training' )
parser.add_argument('--lam-v', default=1.0, type=float,
                    help='hyper-param for vat loss')
parser.add_argument('--epsilon-vat', default=5.0, type=float,                                                          help='hyper-param for vat loss gradient') 
...
def main():
    ...
    val_loader = Data.DataLoader(
        dataset=val_set, batch_size=64, shuffle=False) #changed: 512 --> 64
    test_loader = Data.DataLoader(
        dataset=test_set, batch_size=64, shuffle=False)

    # Define the model, set the optimizer
    model = MixText(n_labels, args.mix_option).cuda() # moved to global , line 105
    embedder = model.bert.embeddings  # used in train() arguments
    ...
    for epoch in range(args.epochs):
        ...
        train(labeled_trainloader, unlabeled_trainloader, model, optimizer,
              scheduler, train_criterion, epoch, n_labels, args.train_aug, embedder=embedder)
        ...
        print(best_acc)
        t1 = time.time()
        print('Epoch {} took {:.2f} sec'.format(epoch, t1-t0))
...
def train(labeled_trainloader, unlabeled_trainloader, model, optimizer, scheduler, criterion, epoch, n_labels, train_aug=False, embedder=None):
    ...
    for batch_idx in range(args.val_iteration):
        ...
        if args.separate_mix: # mixtext: False, Tmix:True
        ...
        if args.use_vat:
            v_loss = vat_loss(embedder, model, inputs_x, perturb_norm_length=args.epsilon_vat,  #perturb_norm_length= epsilon in paper
            small_constant_for_finite_diff=1e-1, Ip=1, p_logit=model(inputs_x))

        # ADD v_loss to other losses
        if mix_ == 1:
            loss = Lx + w * Lu   # v_loss removed 
        else:
            loss = Lx + w * Lu + w2 * Lu2 
